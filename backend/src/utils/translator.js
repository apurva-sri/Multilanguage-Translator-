...
const openaiTranslate = async (text, from, to) => {
...
const translateWithFallback = async (text, from, to) => {
  ...
  if (cached) {
    console.log(chalk.cyan(`⚡ Cache hit for ${from} → ${to}`));
    return cached;
  }
  ...
  try {
    ...
    console.error(
      chalk.yellow.bold(
        "⚠️  OpenAI translation failed, falling back to HuggingFace:"
      ),
      e.message
    );
    try {
      ...
      console.error(
        chalk.red.bold("❌ HuggingFace translation also failed:"),
        e2.message
      );
